  ...
  return (
    <div style={{ maxWidth: 960, margin: '24px auto', padding: 24 }}>
      ...
      <p style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        <Link href="/admin">← Retour tableau de bord</Link>
        <Link href="/admin/offers/new" style={{ marginLeft: 'auto' }}>+ Nouvelle offre</Link>
      </p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {data.data.map((o: AdminOffer) => (
              <tr key={o.id}>
                <td style={{ padding: 8, borderBottom: '1px solid #f0f0f0' }}>{o.title}</td>
                <td style={{ padding: 8, borderBottom: '1px solid #f0f0f0' }}>{o.slug}</td>
                <td style={{ padding: 8, borderBottom: '1px solid #f0f0f0' }}>{o.is_active ? 'Oui' : 'Non'}</td>
                <td style={{ padding: 8, borderBottom: '1px solid #f0f0f0' }}>
                  {o.price ? `${o.price} ${o.price_currency || 'EUR'}` : '-'}
                </td>
                <td style={{ padding: 8, borderBottom: '1px solid #f0f0f0', display: 'flex', gap: 8 }}>
                  <Link href={`/admin/offers/${o.id}`} aria-label="Modifier" title="Modifier" style={{ display: 'inline-flex', alignItems: 'center' }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M12 20h9" />
                      <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z" />
                    </svg>
                  </Link>
                  <button onClick={() => onDelete(o.id)} aria-label="Supprimer" title="Supprimer" style={{ background: 'none', border: '1px solid #eee', borderRadius: 4, padding: 6, cursor: 'pointer', color: 'crimson', display: 'inline-flex', alignItems: 'center' }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
