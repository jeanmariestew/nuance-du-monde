...
export async function GET() {
  ...
  try {
    ...
    const [rows] = await pool.query(`SELECT ${keyCol} AS k, ${valueCol} AS v FROM settings`);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    for (const [key, value] of entries) {
      const [res]: any = await conn.query(`UPDATE settings SET ${valueCol} = ? WHERE ${keyCol} = ?`, [String(value), key]);
      if (res.affectedRows === 0) {
        await conn.query(`INSERT INTO settings (${keyCol}, ${valueCol}) VALUES (?, ?)`, [key, String(value)]);
      }
    }
    await conn.commit();
