...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [rows] = await pool.query('SELECT * FROM travel_themes WHERE id = ?', [id]);
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await pool.query(
      `UPDATE travel_themes SET title=?, slug=?, description=?, short_description=?, image_url=?, banner_image_url=?, sort_order=?, is_active=? WHERE id=?`,
      [title, slug, description, short_description, image_url, banner_image_url, sort_order, is_active ? 1 : 0, id]
    );
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!id) return NextResponse.json({ success: false, error: 'Invalid id' }, { status: 400 });
  try {
    await pool.query('DELETE FROM travel_themes WHERE id = ?', [id]);
    return NextResponse.json({ success: true });
  } catch (e: any) {
    return NextResponse.json({ success: false, error: e.message }, { status: 500 });
  }
