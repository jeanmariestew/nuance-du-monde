...
export async function GET() {
  ...
  const [rows] = await pool.query('SELECT * FROM travel_types ORDER BY created_at DESC');
...
export async function POST(req: Request) {
  ...
  try {
    await pool.query('INSERT INTO travel_types (title, slug, is_active) VALUES (?, ?, ?)', [title, slug, is_active ? 1 : 0]);
