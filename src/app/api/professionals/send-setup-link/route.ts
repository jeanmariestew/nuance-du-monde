...
export async function POST(request: NextRequest) {
  try {
    ...
    const rows = await query(
      'SELECT id, first_name, last_name, agency_name, status FROM professionals WHERE LOWER(email) = LOWER(?)',
      [email.trim()]
    );
    ...
    try {
      ...
      console.error('[send-setup-link] Email error:', emailErr);
      // Ne pas bloquer si l'email échoue
    }

    return NextResponse.json({
      success: true,
      message: 'Un lien de configuration a été envoyé à votre email'
    });

  } catch (error) {
    console.error('[POST /api/professionals/send-setup-link] Error:', error);
    return NextResponse.json(
